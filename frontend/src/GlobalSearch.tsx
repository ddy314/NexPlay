...
export function GlobalSearch({ open, localSubjects, onClose, onOpen }: { open: boolean; localSubjects: Subject[]; onClose: () => void; onOpen: (subject: Subject) => void }) {
  ...
  const results = [...local, ...online.filter((subject) => !local.some((item) => item.providerSubjectId === subject.providerSubjectId))].slice(0, 14);
  ...
  useEffect(() => {
    ...
    setOnline((value) => value.length ? [] : value);
    setActive(0);
  }, [open]);
